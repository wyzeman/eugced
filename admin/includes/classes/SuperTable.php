...
class SuperTable {
    ...
    public function displayTable($tpl) {
        ...
        if ($this->action_add == true) {
            ...
            foreach($this->buttons_add as $button) {
                $form_buttons[] = $button;
            }

            $form_buttons[] = ["name" => "cancel", "icon" => "cancel", "label" => T_("Cancel"), "type" => FormWidget::FORM_BUTTON_LINK, "url" => "?initial"];
    ...
    public function addTabbedPane($title, $content) {
        $this->tabs[] = ["title"=>$title,"content"=>$content];
    }

    public function setButtonsAdd($buttons) {
        $this->buttons_add = $buttons;
    }
}
