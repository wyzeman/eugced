...
function displaySuperTable() {
    ...
    class MySuperTable extends SuperTable
    {
        ...
        public function callbackAddValidate($items, $foreign_items) {
            ...
            if ($items["username"] == "") {
                return ["result"=>false,"error"=>T_("Username is empty!")];
            }

            // Check for duplicate emails
            if ($DB->getCount("tb_users", ["username","=",$items["username"]]) > 0) {
                return ["result"=>false,"error"=>T_("Username already in use!")];
            }
        ...
        public function callbackModifyValidate($items, $foreign_items, $modify_id) {
            ...
            // Check for duplicate emails
            if ($DB->getCount("tb_users", [["username","=",$items["username"]],"AND",["id","!=",$modify_id]]) > 0) {
                return ["result"=>false,"error"=>T_("Username already in use!")];
            }
